...
sys.path.append('..')
from tfomics.layers import MultiHeadAttention, RevCompConv1D, RevCompMaxPool
...
def model(input_shape, num_labels, filters, dims, num_heads=12, num_layers=4, pool_size=5, 
  ...
  # layer 1 - convolution
  nn, nn2 = keras.layers.RevCompConv1D(filters=filters, kernel_size=19, use_bias=True, padding='same',
  ...
  forward_layer2 = keras.layers.LSTM(dims//2, return_sequences=True)
  backward_layer2 = keras.layers.LSTM(dims//2, activation='relu', return_sequences=True, go_backwards=True)
  nn2 = keras.layers.Bidirectional(forward_layer2, backward_layer=backward_layer)(nn2)
